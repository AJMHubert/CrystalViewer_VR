...
void CellGeometry::calculateCartesianBasis()
{
    ...
    double brad = beta * (PI / 180);
    double grad = gamma * (PI / 180);

    // easy enough
    avec = Eigen::Vector3d(a, 0.0, 0.0);
    // slightly trickier
    bvec = Eigen::Vector3d(b*std::cos(grad), b*std::sin(grad), 0.0);
    // WOAH
    double cv1 = c * std::cos(brad);
    double cv2 = c * (std::cos(arad) - std::cos(brad) *std::cos(grad)) / std::sin(grad);
    double cv3 = c * std::sqrt( (std::cos(brad-grad) - std::cos(arad)) * (std::cos(arad) - std::cos(brad + grad)) ) / std::sin(grad);
